...
// FIX: authClient.getSession() resolves to { data, error }, not the
// session object directly. The old code read session?.user?.id, which was
// always undefined -> empty x-user-id/x-user-email headers -> backend
// requireAuth always returned 401. Centralized here so both handleSave and
// handleDelete build headers the same (correct) way.
