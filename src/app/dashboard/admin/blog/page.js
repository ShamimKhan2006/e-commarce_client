...
async function getAuthHeaders() {
  ...
  return {
    ...
    "x-user-role": data?.user?.role || "user",
  };
}
...
export default function BlogManagePage() {
  ...
  const handleDelete = async (p) => {
    ...
    try {
      // FIX: this request used to send no auth headers at all, so it
      // always hit the backend's requireAuth as 401.
      const headers = await getAuthHeaders();
      // FIX: backend returns raw MongoDB documents with `_id`, not `id`.
      // Using p.id here was always undefined, so the DELETE request went
      // to /api/blogs/undefined and failed. Use p._id instead.
      const res = await fetch(`/api/blogs/${p._id}`, {
        method: "DELETE",
        headers,
      });
      if (!res.ok) throw new Error();
